defmodule MediaCentaur.Library.Progress.Worker do
  @moduledoc """
  ...
  hot path — `MediaCentaur.Playback.MpvSession` records position ticks
  ...
  Reads bypass the GenServer entirely — see `MediaCentaur.Library.Progress.get/1`.
  ...
  ## Lifecycle
    ...
    * `init/1` opens the ETS table (named per `:table` arg, default
      ...
      `MediaCentaur.Library.Progress.Events.broadcast/1` (the typed-event
  ...
  `MediaCentaur.Library.Progress.Events.broadcast/1` on the
  ...
  ## Configuration
    ...
    * `:flush_interval_ms` — overrides the application env key
      `:media_centaur, :library_progress_flush_interval_ms`. Tests
  ...
  alias MediaCentaur.Library.Progress.Events
  alias MediaCentaur.Library.Progress.Events.{ProgressFlushed, ProgressHydrated, ProgressTicked}
            ...
            in_memory ->
              {:ok, updated} =
                MediaCentaur.Repo.update(
                  MediaCentaur.Library.WatchProgress.update_changeset(record, %{

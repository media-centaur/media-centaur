defmodule MediaCentaur.Library.Deletion do
  @moduledoc """
  Removes files from the library.

  Two shapes of caller share one cleanup cascade:

    * **User-initiated** — `delete_file/1`, `delete_files/1` and
      `delete_folder/2` remove the files from disk first, then clean
      the records and broadcast `entities_changed` once per call.
    * **Already gone** — `cleanup_removed_files/1` cleans records for
      paths that disappeared on their own (inotify deletions via
      `Library.FileEventHandler`, TTL expiry via `Library.AbsenceSweeper`).

  The cascade groups removed files by top-level entity, deletes the
  matching child records (episodes, movies, extras, images, playable
  items, progress), and destroys the entity outright when no
  `WatchedFile` remains.
  """
  ...
  # ---------------------------------------------------------------------------
  # Disk deletion — user-initiated, from the detail modal and Review
  # ---------------------------------------------------------------------------
  ...
  # ---------------------------------------------------------------------------
  # Record cleanup — shared by the paths above and by file-removal events
  # ---------------------------------------------------------------------------

defmodule MediaCentaur.Library.PlayableItem do
  ...
  enforced at the write seam (`MediaCentaur.Library.Inbound` and the
  `Library` context's `create_playable_item/1`); orphan rows would only
  appear if writes skip the boundary.

  See ADR-047 (PlayableItem reification) for the rationale and target shape.
  """
  ...
  def create_changeset(attrs) do
    ...
    # `Ecto.ConstraintError`. See `MediaCentaur.Library.ExternalId` for
    # the same pattern.

defmodule MediaCentaur.Pipeline.EntityImageContext do
  ...
  it can (re)fetch artwork: its **TMDB id** and a **media_dir**.
  ...
  Task B), so a media_dir is found by walking `library_playable_items`.
  """
  import Ecto.Query
  ...
  def find_media_dir(episode_id, :episode) do
    ...
    direct =
      Repo.one(
        from(wf in WatchedFile,
          ...
          where: pi.container_id == ^episode_id and not is_nil(wf.media_dir),
          select: wf.media_dir,
          limit: 1
        )
      )
    ...
    case direct do
      media_dir when is_binary(media_dir) ->
        {:ok, media_dir}
  ...
  def find_media_dir(entity_id, :movie) do
    ok_or_skip(
      Repo.one(
        from(wf in WatchedFile,
          join: pi in PlayableItem,
          on: pi.id == wf.playable_item_id and pi.container_type == :movie,
          where: pi.container_id == ^entity_id and not is_nil(wf.media_dir),
          select: wf.media_dir,
          limit: 1
        )
      )
  ...
  def find_media_dir(entity_id, :video_object) do
    ok_or_skip(
      Repo.one(
        from(wf in WatchedFile,
          join: pi in PlayableItem,
  ...
  def find_media_dir(tv_series_id, :tv_series) do
    ok_or_skip(
      Repo.one(
        from(wf in WatchedFile,
          join: pi in PlayableItem,
          ...
          join: s in Season,
          on: s.id == e.season_id,
          where: s.tv_series_id == ^tv_series_id and not is_nil(wf.media_dir),
          select: wf.media_dir,
          limit: 1
        )
      )
  ...
  def find_media_dir(movie_series_id, :movie_series) do
    ok_or_skip(
      Repo.one(
        from(wf in WatchedFile,
          join: pi in PlayableItem,
          ...
          where: m.movie_series_id == ^movie_series_id and not is_nil(wf.media_dir),
          select: wf.media_dir,
          limit: 1
        )
      )
  ...
  defp ok_or_skip(media_dir) when is_binary(media_dir), do: {:ok, media_dir}
  defp ok_or_skip(_), do: {:skip, :no_media_dir}
end

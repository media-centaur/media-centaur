defmodule MediaCentaur.Pipeline.Stats do
  ...
  @doc false
  def handle_telemetry([:media_centaur, :pipeline, :stage, :start], _measurements, metadata, config) do
  ...
  def handle_telemetry([:media_centaur, :pipeline, :stage, :exception], measurements, metadata, config) do
    stage_exception(
      config.stats,
      metadata.stage,
      measurements.duration,
  ...
  def handle_telemetry([:media_centaur, :pipeline, :needs_review], _measurements, metadata, config) do

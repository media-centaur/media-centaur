defmodule MediaCentaur.ReleaseTracking.RefreshSchedule do
  ...
  immediately, no prior cycle to wait for. `now` is injectable so tests
  can assert exact delays instead of padding for wall-clock skew.
  """
  @spec next_delay_ms(DateTime.t() | nil, non_neg_integer(), DateTime.t()) :: non_neg_integer()
  def next_delay_ms(last_completed_at, interval_ms, now \\ DateTime.utc_now())

  def next_delay_ms(nil, _interval_ms, _now), do: 0

  def next_delay_ms(%DateTime{} = last_completed_at, interval_ms, %DateTime{} = now)

defmodule MediaCentaur.SelfUpdate.Storage do
  @moduledoc """
  Durable storage for SelfUpdate state — `last_check_at` and `latest_known`
  release metadata — persisted via `MediaCentaur.Settings.Entry`.
  ...
  def record_check_result({:ok, release}) do
    classification = UpdateChecker.compare(release, MediaCentaur.Version.current_version())

defmodule MediaCentaur.Platform.DriveProbe do
  @moduledoc """
  Drive-capacity probe — behaviour + facade.

  Decouples `MediaCentaur.Storage` from the OS-specific way to ask
  ...
  * **macOS** uses BSD `df -k -P` (POSIX format, 1024-block units)
  ...
  ## Usage
      ...
      iex> MediaCentaur.Platform.DriveProbe.available_bytes("/tmp")
      {:ok, _bytes}
  ...
  defp impl do
    MediaCentaur.Platform.pick_impl(__MODULE__,
      linux: MediaCentaur.Platform.DriveProbe.GnuDf,
      darwin: MediaCentaur.Platform.DriveProbe.BsdDf
    )
  end
end

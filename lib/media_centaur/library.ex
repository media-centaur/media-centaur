defmodule MediaCentaur.Library do
  ...
  def playable_item_ids_for_entities(entity_ids) when is_list(entity_ids) do
    ...
    via_tv_series =
      from(p in PlayableItem,
        join: e in MediaCentaur.Library.Episode,
  ...
  **presence-agnostic** — entities are included regardless of whether
    ...
    with {:ok, updated} <- Repo.update(WatchProgress.mark_completed_changeset(progress)) do
      if transitioning? do
        Phoenix.PubSub.broadcast(
          MediaCentaur.PubSub,
          MediaCentaur.Topics.library_watch_completed(),
  ...
  (`MediaCentaur.WatchHistory.reset_watch_progress/1`,
  ...
  `MediaCentaur.Library.Progress.Worker` during the debounced flush —
  ...
  (`MediaCentaur.Library.EpisodeList.progress_container_id/1`) can key
  ...
  (`MediaCentaurWeb.ViewModel.SeriesDetail.compose/1`,
  `MediaCentaur.Library.load_modal_entry/1`) after the Phase 3.2
  ...
  Used by `MediaCentaurWeb.LibraryLive` to populate the `progress_by_id`

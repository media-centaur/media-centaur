defmodule MediaCentaur.Console.Entry do
  @moduledoc """
  ...
  @doc """
  Builds an `%Entry{}` from a raw Erlang `:logger` event — the level, the
  message tuple, and the metadata map.

  This is the shared construction used by both the volatile `Console.Handler`
  and the durable `ErrorReports.LogHandler`, so the two independent log paths
  classify components, render messages, and prune metadata identically. Pure:
  no PubSub, no DB.
  """
  @spec from_log_event(atom(), term(), map()) :: t()
  def from_log_event(level, msg, meta) do
    %__MODULE__{

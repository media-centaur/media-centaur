defmodule MediaCentaur.Showcase do
  ...
  Entry point: `MediaCentaur.Showcase.seed!/0`. The Mix task
  ...
  **Every title string surfaced by the showcase must be public-domain or
  ...
  library catalog:
    ...
    * `MediaCentaur.Showcase.Catalog` — movies, TV series, video objects.
  ...
  Parser regression tests under `test/media_centaur/parser_test.exs` are
  ...
  All TMDB lookups go through `MediaCentaur.TMDB.Client` which honours the
  ...
  defp track_upcoming(client, :movie, title) do
    case TMDB.Client.search_movie(title, nil, client: client) do
      {:ok, [%{"id" => id, "title" => name} | _]} ->
        ...
        case ReleaseTracking.set_rung(title, :grab) do
          {:ok, _intent} -> :ok
          _ -> :error
        end
  ...
  defp mark_queue_complete(owner_id, role) do
    ...
    MediaCentaur.Repo.update_all(
      from(e in MediaCentaur.Pipeline.ImageQueueEntry,

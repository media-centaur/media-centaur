defmodule MediaCentaurWeb.Components.UpcomingCards do
  ...
  defmodule TrackedItem do
    ...
    `MediaCentaur.ReleaseTracking.Item` plus its release counts.
    """
  ...
  @doc_events_list "list of `MediaCentaur.ReleaseTracking.Event.t()` rows."
  ...
  attr :release, :any,
    required: true,
    doc:
      "single `MediaCentaur.ReleaseTracking.Release.t()` — preloaded with `:item`. The owning context's boundary doesn't export the schema module, so the attr stays loose with this prose contract."
  ...
  attr :release, :any,
    required: true,
    doc:
      "single `MediaCentaur.ReleaseTracking.Release.t()` — preloaded with `:item`. See `release_tile/1` for the prose contract rationale."
  ...
  attr :item, :map,
    required: true,
    doc:
      "tracked `MediaCentaur.ReleaseTracking.Item.t()` (the schema, not the view-model). Boundary doesn't export the module; contract is documented prose."
  ...
  attr :item, :map,
    required: true,
    doc:
      "tracked `MediaCentaur.ReleaseTracking.Item.t()`. Same boundary-prose contract as `active_card/1`."
  ...
  attr :item, :map,
    default: nil,
    doc:
      "the `MediaCentaur.ReleaseTracking.Item.t()` pending stop-confirmation, or `nil` when the modal is closed. Template only reads `.name`."
  ...
  the resolver doesn't pull `MediaCentaur.ReleaseTracking.Release` across
  ...
  def release_status(false, %Target{status: "acquired"}, %QueueItem{state: state, health: health}) do
    case state do
      ...
      :downloading ->
        if MediaCentaur.Downloads.Health.degraded?(health),
  ...
  `MediaCentaur.Acquisition.statuses_for_releases/1`.
  """

defmodule MediaCentaurWeb.Components.Incoming.Shelf do
  ...
  entry (the ledger's "Show earlier" idiom, applied to the shelf). The
  shelf is the schedule and nothing else: a tracked title with no
  announced date is not a row here — it shows its mode where it lives,
  in the library or on the watchlist (UIDR-035). An empty forecast
  renders nothing at all — no dead panel; the omnibox above is the
  standing affordance.

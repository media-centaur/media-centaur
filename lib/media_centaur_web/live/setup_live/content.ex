defmodule MediaCentaurWeb.Live.SetupLive.Content do
  ...
  def for(:watch_dirs) do
    %__MODULE__{
      ...
      requirements: [
        ...
        "Read access to that folder for the user running Media Centaur"
      ]
    }
  end
  ...
  def for(:tmdb) do
    %__MODULE__{
      ...
      what:
        "The Movie Database is the source of all metadata Media Centaur displays — titles, " <>
  ...
  def for(:mpv) do
    %__MODULE__{
      ...
      what:
        "mpv is the playback engine Media Centaur launches when you press Play. The app " <>
      ...
      requirements: [
        ...
        "An absolute path to the binary — Media Centaur can auto-detect common locations"
      ]
    }
  end
    ...
    %__MODULE__{
      ...
      what:
        "ffprobe (part of the FFmpeg toolkit) reads the metadata stream of a video file. " <>
          "Media Centaur uses it to discover embedded subtitle tracks inside MKV/MP4 containers " <>
    ...
    %__MODULE__{
      ...
      what:
        "Prowlarr is a separate self-hosted service that aggregates torrent / Usenet indexers " <>
          "behind a single API. Media Centaur talks to it for searches initiated from the " <>
    ...
    %__MODULE__{
      ...
      what:
        "Once Prowlarr forwards a grab to a download client (qBittorrent, Transmission, etc.), " <>
          "Media Centaur talks to that client directly to read progress: percentage, ETA, status. " <>

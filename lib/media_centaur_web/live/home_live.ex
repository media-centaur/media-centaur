defmodule MediaCentaurWeb.HomeLive do
  ...
  alias MediaCentaur.ReleaseTracking.Views, as: ReleaseTrackingViews
  ...
  defp assign_empty_sections(socket) do
    ...
    |> assign(:coming_up_marquee, %MediaCentaurWeb.Components.ComingUpMarquee.Marquee{
      hero: nil,
      secondaries: []
    })
  ...
  def handle_info({:playback_failed, %{payload: payload}}, socket) do
    {:noreply,
     put_flash(
       socket,
       :error,
       MediaCentaurWeb.LibraryFormatters.playback_failed_flash(payload)
     )}
  end

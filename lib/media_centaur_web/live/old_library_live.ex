defmodule MediaCentaurWeb.OldLibraryLive do
  ...
  def handle_event("switch_tab", %{"tab" => tab}, socket) do
    ...
    {:noreply,
     push_patch(socket,
       to:
         ~p"/old-library?#{build_params(tab, socket.assigns.selected_id, socket.assigns.filter_text)}"
     )}
  end
  ...
  def handle_event("filter", %{"filter_text" => text}, socket) do
    {:noreply,
     push_patch(socket,
       to:
         ~p"/old-library?#{build_params(socket.assigns.active_tab, socket.assigns.selected_id, text)}",
       replace: true
     )}
  end
  ...
  def handle_event("select_entity", %{"id" => id}, socket) do
    ...
    {:noreply,
     push_patch(socket,
       to:
         ~p"/old-library?#{build_params(socket.assigns.active_tab, new_id, socket.assigns.filter_text)}"
     )}
  end
  ...
  def handle_info(:reload_entities, socket) do
    ...
    if selection_deleted do
      {:noreply,
       push_patch(socket,
         to:
           ~p"/old-library?#{build_params(socket.assigns.active_tab, nil, socket.assigns.filter_text)}"
       )}
    else
      {:noreply, socket}
    end
  ...
  def render(assigns) do
    ~H"""
    <Layouts.app flash={@flash} current_path="/old-library">
      <div class="space-y-4">
        <h1 class="text-2xl font-bold">Old Library</h1>

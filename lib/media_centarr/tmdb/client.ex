defmodule MediaCentarr.TMDB.Client do
  @moduledoc """
  ...
  defp build_client do
    api_key = MediaCentarr.Config.get(:tmdb_api_key) |> MediaCentarr.Secret.expose()
  ...
  defp get(client, opts) do
    ...
    MediaCentarr.TMDB.RateLimiter.wait()
    wait_duration = System.monotonic_time() - wait_start

    :telemetry.execute(
      [:media_centarr, :tmdb, :rate_limit_wait],
      %{duration: wait_duration},
      %{endpoint: endpoint}
    )

    :telemetry.span([:media_centarr, :tmdb, :request], %{endpoint: endpoint}, fn ->

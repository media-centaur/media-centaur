defmodule MediaCentarr.Platform.Autostart.Launchd do
  @moduledoc """
  macOS implementation of `MediaCentarr.Platform.Autostart`.

  Wraps `launchctl` for the LaunchAgent that supervises the running
  media-centarr release. The agent's plist (which the macOS
  installer drops into `~/Library/LaunchAgents/`) sets the Label to
  `com.media-centarr.app` — that's the identifier every `launchctl`
  call here targets.
  ...
  * **State**: `launchctl print gui/<uid>/<label>` — exits 0 if
  ...
  @handoff_env_vars []
  @tarball_required_paths ["share/launchd/com.media-centarr.app.plist"]

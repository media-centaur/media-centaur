defmodule MediaCentarr.Pipeline.Stages.Search do
  @moduledoc """
  ...
  defp search(title, year, :unknown) do
    movie_task =
      Task.Supervisor.async(MediaCentarr.TaskSupervisor, fn ->
        Client.search_movie(title, year)
      end)

    tv_task =
      Task.Supervisor.async(MediaCentarr.TaskSupervisor, fn -> Client.search_tv(title, year) end)

defmodule MediaCentarrWeb.ConsoleLive.Logic do
  @moduledoc """
  ...
  Shared by `MediaCentarrWeb.ConsoleLive` (sticky drawer) and
  `MediaCentarrWeb.ConsolePageLive` (full-page `/console` route) so both
  ...
  alias MediaCentarr.Console.{Buffer, Entry, Filter, View}
  ...
  def download_filename(%DateTime{} = now \\ DateTime.utc_now()) do
    "media-centarr-#{Calendar.strftime(now, "%Y-%m-%dT%H-%M-%S")}.log"
  end

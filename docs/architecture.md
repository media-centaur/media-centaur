...
| `MediaCentaur.TMDB` | TMDB HTTP adapter + rate limiter + the TMDB store (`tmdb_titles`, `tmdb_seasons` — one record per title, ADR-071) | Cross-cutting adapter, not a bounded context owner. The store is the only detail caller (Credo MC0038); its checks run from `TMDB.CheckJob`. |
...
PubSub listener GenServers (`Library.Inbound`, `Review.Intake`, `ReleaseTracking.LibraryListener`, `ReleaseTracking.TmdbListener`, `Pipeline.TmdbProjection`, `WatchHistory.Recorder`, `Acquisition.Reactor`, `Downloads.QueueMonitor`, `Pursuits.InboundListener`) are skipped in `:test` env, and the Oban cron jobs (`TMDB.CheckJob`, `ReleaseTracking.SweepJob`, the pursuit watcher, the sweeps) run only when a test performs them — tests call the public functions directly. Watchers and the pipelines start in disabled state in tests; production toggles them via `services:<env>:start_watchers` / `start_pipeline` keys in `Settings`.
...
- **Artwork is warm before it is asked for.** The root layout prefetches the first-screen posters and the page hero backdrops at launch (`MediaCentaurWeb.ArtworkWarmup`), and `app.js` pre-decodes the heroes into a JavaScript-owned `ImageBitmap` cache that survives live navigation; `Components.HeroBackdrop` paints them onto a canvas in the mount frame. Every URL on both sides comes from one function per surface (`poster_src/1`, `hero_backdrop_src/1`) so the hints and the pages can never disagree. See [UIDR-012](../decisions/user-interface/2026-05-20-012-desktop-app-rendering-defaults.md) and [UIDR-032](../decisions/user-interface/2026-09-06-032-page-hero-backdrops-paint-from-a-decoded-bitmap-cache.md).

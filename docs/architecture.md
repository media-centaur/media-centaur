...
| `MediaCentaur.Friends` | no tables yet — this install's Nostr identity in the sensitive `nostr_secret_key` config key | The friend network's configuration: `Friends.Identity` (one secp256k1 keypair, generated the first time the Friends tab is opened) and, in later layers, the relay list and the friend roster. Broadcasts on `friends:updates`. |

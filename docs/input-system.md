...
| `data-nav-dismiss-event` | On a zone container: the LiveView event BACK pushes when it leaves the zone along its `back` edge — a menu list closes itself this way | `title_menu_close`, `close_sort` |

...
**Overlays with regions.** An overlay carrying `data-nav-overlay="<name>"` navigates as several zones instead of one flat list, per `config.overlays[name]`: `entry` is the cursor-start priority among its regions, `layout` its internal edges (merged over the page's graph while it is open). The detail modal declares `detail` — a `detail_actions` TOOLBAR over the body, whose zone follows the sub-view: the season/film/extras lists and the Manage ledger are a `detail_list` TREE, the Cast view a `detail_cast` SHELF (a photo grid, so adjacency is answered by geometry — the lead/other-episodes sections and the Show more button need no adjacency table). The Manage sub-view additionally interposes `manage_tools`, a TOOLBAR for its toolbar card: the card is a horizontal strip (Delete all, Rematch, Refresh artwork, the ID links), so LEFT/RIGHT move along it and DOWN drops past it into the ledger — walked as tree items, DOWN used to step sideways through it. Only one body zone is in the DOM at a time (`manage_tools` is empty outside Manage); the candidate lists on the `down` edges route to whichever region is populated, and each region's `back` edge gives BACK its first rung. The cast grid and `manage_tools` declare `up: ["detail_actions"]` — a spatial region has a geometric "above" — while the tree deliberately does not (there, BACK is the way out; see UIDR-019). Overlays without the attribute stay flat MODAL, which is right for a confirm or a small form.

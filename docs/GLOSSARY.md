...
| **Recommendations** (tab) | The Discovery tab listing recommendations newest first in two scopes: **Incoming** (from friends, the default) and **Yours** (this install's, each with Delete). Reading only; adding to the watchlist is the user's act. Recommending happens on a title's detail page. |
| **Tombstone** | A withdrawn recommendation kept as a row with `deleted_at` and the signed deletion event (kind 5), so a stale copy from a relay stays hidden and the deletion can be republished. A newer recommendation for the same address revives it. |
| **Discovery** | The page where titles come to the user: Recommendations, Watchlist and Friends tabs at `/discovery`, gated in the sidebar by the `show_discovery` preference. |

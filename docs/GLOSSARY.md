...
| **Social** | The subsystem that connects one install to friends' installs over Nostr relays: identity, relays, friends, and the recommendations that travel between them. Bounded context `MediaCentaur.Social` (configuration) plus `MediaCentaur.Recommendations` (content) over `MediaCentaur.Nostr` (protocol). Status tile, incident component and console tag `:social`; the wire logs as `:nostr`. Discovery's third tab. |
| **Friend** | One entry on the Social roster: a public key the user follows under a locally chosen nickname (`Social.Friend`, `friends` table). Friends are the only authors the feed accepts. Never the name of the subsystem. |
...
| **Discovery** | The page where titles come to the user: Feed, Watchlist and Social tabs at `/discovery`, gated in the sidebar by the `show_discovery` preference. |

...
- [ ] Commit `feat(pipeline): import reads the TMDB store — first contact for a new title, the stored copy for an owned one`.
...
- [ ] Commit `feat(pipeline): the library is re-projected from the TMDB store when a title changes`.

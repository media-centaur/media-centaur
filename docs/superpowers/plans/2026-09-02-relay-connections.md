...
- Connection state atoms: `:connecting | :connected | :disconnected | :auth_failed`. Owner messages: `{:nostr, url, msg}` with `msg` one of `:connected`, `{:disconnected, reason}`, `{:event, sub_id, %Event{}}`, `{:eose, sub_id}`, `{:ok, event_id, accepted?, reason}`, `{:notice, text}`, `{:auth, :ok | {:failed, reason}}`.
...
- Relay URL validation: scheme `ws` or `wss`, non-empty host, no userinfo; stored as typed with the trailing slash trimmed. Errors surface as a flash: **"Relay addresses start with wss:// or ws://"**.

...
**Architecture:** Pillar-1 structural redesign. The campaign target schema (`library-schema-v2` "Target schema" section) is the spec. After Phase 2, every supporting table either keys to `playable_item_id` (file/progress/subtitle) or uses a single `(owner_type, owner_id)` polymorphic pair (image/extra/external_id). One container can host N playable items (unlocks director's cuts, multi-part episodes).
...
**Campaign reference:** `library-schema-v2`. Phase 1 complete; this plan executes Phase 2.

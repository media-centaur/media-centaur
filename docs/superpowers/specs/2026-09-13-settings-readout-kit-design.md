...
25. **A config change re-seeds from the persisted test, never to `:pending`.** A person's save clears the persisted test *before* it writes a field (`Capabilities.save_integration/2`, likewise `clear_integration/1`), so the per-key broadcasts that follow re-seed the row to `:unknown`; the boot-time config load leaves it in place, so the row keeps its last result (found at implementation: the owner seeds before the runtime config is loaded, and the per-key broadcasts that follow must not erase the seed). A verify in flight keeps `:pending` through a re-seed; `:pending` means a verify is in flight and nothing else. The setup gate is unchanged: `:unknown` blocks the TMDB step until the verify the tour already runs on save.

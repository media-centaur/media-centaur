...
5. **The mode is a URL param.** `mode` is `auto_select_best_release` or `manually_select_release`, parsed by the one wire-form parser the Download event already needs (coherence pass, 3). Incoming resolves it once — an absent param means `PlanningMode.value()`, the person's default — and holds it in the plan param identity; any other value is a malformed plan link: the modal closes and a flash says so. Only the title detail sends the param. "Download more of this show" sends none and gets the default, so every Download in the app performs the planning mode.

### The picker performs the mode

6. **`plan_create` stamps the approval policy from the resolved mode** (`PlanningMode.approval_policy/1`) for both the targeting stage and the movie confirm. The hard-coded `review` goes.
...
## Follow-up (2026-09-23, same day)

Three small changes on top of the shipped feature, approved as one plan.

1. **Starting a download points Incoming at Activity.** Activity is where a started download appears (UIDR-015). When a download starts under auto-select, Incoming shows Activity now if the person is on it — one patch, which also dismisses whichever modal was up — and, from any other page, the sidebar's Incoming entry opens on Activity until the person picks another tab there. The precedent is two existing pieces: approving a plan already lands on Activity, and the sidebar entry already reopens the page's last URL (`data-nav-remember`, session storage), the zone tab included. One new host callback, `download_started/1`, beside `open_plan/2`: Incoming patches to the Activity tab (Approve reuses it); Home, Library and Discovery close the title detail. One pushed browser event, `nav-remember`, handled beside the sidebar-remember script in the root layout, writes the Incoming entry's remembered URL — the same store, a second writer. The Activity tab's path is one pure function on `IncomingLive.Logic` (`zone_path/1`), shared by the tab switch, the callback and the remember event. `PlanFlow.land_started/3` is the auto ending: the flash, the remember, then `:leave` (the host's `download_started/1`) or `:stay` (the gap row and the feed row, which have no modal to drop). LiveView refuses two redirects in one reply, so the ending is one act. `Acquisition.start_download/5` and `PlanFlow.land_plan/5` take the ending word instead of a close function.
2. **Clear selected.** The picker's None preset reads *Clear selected*; its wire value is `clear` so the code name matches the copy.
3. **The picker's Download is the split button.** `GlassMenu.split_button`, the title detail's control, in both the targeting and movie-confirm footers from one private function. The main segment performs the mode the picker was opened with — the link's, else the person's default (decision 5) — and the menu names the other, sending it as `plan_create`'s `mode`; `PlanningMode.parse_mode/2` resolves a click's mode against a default for the picker and the title detail alike. The host owns the menu's open state (`plan_menu_open?`, reset on every open and close of the modal). With nothing selected the split is inert (`aria-disabled`, dimmed) rather than disabled, so the nav graph never shifts. The open list is the `plan_menu` TREE nested in `plan_body`, the plan overlay's one `back` edge. The footer sits at the bottom of a scrolling body, so the glass menu gained `placement="above"` and `align="end"`; without them the list dropped out of the scrollport.

Rejected: the main segment always performing the Settings default. That would make the mode carried in the link meaningless and the title detail's menu item under *Choose episodes* a lie — it would open the same picker as the main segment. Left alone: the movie-confirm stage is reachable only by typing its URL today; it keeps the same footer and is a candidate for removal on its own.

...
6. The bookmark's tooltip and accessible name are one constant, **Toggle on watchlist** (owner's words); `aria-pressed` carries the state. The tooltip's gap beneath an anchor is 8px (14px stays beside the rail).
7. **Recent activity is gone** from both title surfaces (owner: "I don't think we need recent activity at all"). `TrackingDetail` no longer reads the event log. The log itself — `ReleaseTracking.Event`, written on onboarding, calendar diffs and completion, pruned after 90 days — had no reader left, and is deleted: schema, table (migration `20260914130000`), the calendar `Differ` that existed to write it, and the "Tracking activity log" retention policy.
8. The switches sit on the left of the card and the dates on the right (owner: "the two watchlist controls should be on the left, and the schedule should be on the right").
9. The tooltip reveals on focus only under keyboard or gamepad steering (`focusReveals/1`), the focus-ring rule: a pointer click that opens a modal lands programmatic focus on its first action, and the bookmark's tooltip was popping there.

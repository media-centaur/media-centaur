...
7. **Recent activity is gone** from both title surfaces (owner: "I don't think we need recent activity at all"). `TrackingDetail` no longer reads the event log. The log itself — `ReleaseTracking.Event`, written on onboarding, calendar diffs and completion, pruned after 90 days — now has no reader; deleting it (schema, table, `Differ`, the retention policy) is the next cut, pending the owner.

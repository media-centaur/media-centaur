...
- §12 (reversed after owner review): the modal opens from the snapshot and then fetches the live TMDB detail on open (`start_async`, gated on `Capabilities.tmdb_ready?/0`) into `Components.Detail.TitlePreview` — the plan modal's movie-confirm preview promoted to a shared, media-type-agnostic view-model with `movie/3` and `tv/3` builders — rendered through the shared `Detail.PreviewBody` (metadata row, facets, top cast) with the backdrop, logo and tagline in the frame. The snapshots carry no backdrop, so without the fetch the modal had nothing to dress itself with.

defmodule MediaCentarr.Platform.AutostartTest do
  ...
  describe "facade dispatch" do
    setup do
      original = Application.get_env(:media_centarr, Autostart)
      Application.put_env(:media_centarr, Autostart, FakeImpl)

      on_exit(fn ->
        if original do
          Application.put_env(:media_centarr, Autostart, original)
        else
          Application.delete_env(:media_centarr, Autostart)
        end
      end)
  ...
  describe "default impl" do
    test "defaults to Systemd when no impl is configured" do
      ...
      on_exit(fn ->
        if original, do: Application.put_env(:media_centarr, Autostart, original)
      end)

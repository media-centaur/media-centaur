defmodule MediaCentarr.Platform.LogSourceTest do
  ...
  defmodule FakeImpl do
    @behaviour MediaCentarr.Platform.LogSource
  ...
  describe "facade dispatch" do
    setup do
      original = Application.get_env(:media_centarr, LogSource)
      Application.put_env(:media_centarr, LogSource, FakeImpl)

      on_exit(fn ->
        if original do
          Application.put_env(:media_centarr, LogSource, original)
        else
          Application.delete_env(:media_centarr, LogSource)
        end
      end)
    ...
    test "open_port/1 delegates to the configured impl" do
      assert LogSource.open_port("media-centarr.service") == :fake_port_sentinel
    end

    test "available?/1 returns true for a non-nil unit through the fake impl" do
      assert LogSource.available?("media-centarr.service") == true
    end
  ...
  describe "default impl (no Application env)" do
    test "defaults to Journal when no impl is configured" do
      ...
      on_exit(fn ->
        if original, do: Application.put_env(:media_centarr, LogSource, original)
      end)

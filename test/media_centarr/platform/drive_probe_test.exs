defmodule MediaCentarr.Platform.DriveProbeTest do
  ...
  describe "facade dispatch" do
    test "delegates to the configured impl" do
      original = Application.get_env(:media_centarr, DriveProbe)
      Application.put_env(:media_centarr, DriveProbe, FakeImpl)

      on_exit(fn ->
        if original do
          Application.put_env(:media_centarr, DriveProbe, original)
        else
          Application.delete_env(:media_centarr, DriveProbe)
        end
      end)
    ...
    test "defaults to GnuDf when no impl is configured" do
      ...
      on_exit(fn ->
        if original, do: Application.put_env(:media_centarr, DriveProbe, original)
      end)

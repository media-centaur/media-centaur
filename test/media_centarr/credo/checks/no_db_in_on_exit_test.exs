defmodule MediaCentarr.Credo.Checks.NoDbInOnExitTest do
  use Credo.Test.Case, async: true

  alias MediaCentarr.Credo.Checks.NoDbInOnExit
  ...
  describe "clean code (negative cases)" do
    ...
    test "non-test paths are not analysed" do
      ~S'''
      defmodule MediaCentarr.SomeModule do
        def some_function do
          on_exit(fn -> Config.update(:x, :y) end)
        end
      end
      '''
      |> to_source_file("lib/media_centarr/some_module.ex")
      |> run_check(NoDbInOnExit)
      |> refute_issues()
    end
  end
  ...
  describe "violations (positive cases)" do
    ...
    test "MediaCentarr.Config.update inside on_exit is reported (qualified call)" do
      ~S'''
      defmodule SomeTest do
        test "x" do
          on_exit(fn -> MediaCentarr.Config.update(:k, :v) end)
        end
      end
      '''
      |> to_source_file("test/some_test.exs")

defmodule MediaCentarr.PlatformTest do
  ...
  describe "pick_impl/3 — Application env override wins" do
    setup do
      original = Application.get_env(:media_centarr, FakeFacade)
      Application.put_env(:media_centarr, FakeFacade, FakeDarwin)

      on_exit(fn ->
        if original do
          Application.put_env(:media_centarr, FakeFacade, original)
        else
          Application.delete_env(:media_centarr, FakeFacade)
        end
      end)
  ...
  describe "pick_impl/3 — OS-based default when no override" do
    setup do
      ...
      on_exit(fn ->
        if original, do: Application.put_env(:media_centarr, FakeFacade, original)
      end)

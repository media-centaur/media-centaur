defmodule MediaCentarr.Watcher.SupervisorTest do
  ...
  setup do
    ...
    original_config = :persistent_term.get({MediaCentarr.Config, :config})
    ...
    on_exit(fn ->
      ...
      :persistent_term.put({MediaCentarr.Config, :config}, original_config)
      File.rm_rf!(tmp_dir)
    end)
  ...
  defp drain_task_supervisor(attempts \\ 20) do
    if Task.Supervisor.children(MediaCentarr.TaskSupervisor) != [] and attempts > 0 do
      Process.sleep(50)

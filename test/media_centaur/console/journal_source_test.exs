defmodule MediaCentaur.Console.JournalSourceTest do
  ...
  describe "available?/1" do
    ...
    test "returns true when a unit is detected" do
      {_, name} =
        start_source(
          unit_fetcher: static_unit("media-centaur-dev.service"),
  ...
  describe "subscribe/1" do
    ...
    test "spawns the port on first subscriber and returns an empty snapshot" do
      ...
      assert_receive {:port_opened, "media-centaur-dev.service", _port_ref}, 500
    end
  ...
  describe "broadcast on each line" do
    test "publishes {:journal_line, entry} on Topics.service_journal()" do
      Phoenix.PubSub.subscribe(MediaCentaur.PubSub, Topics.service_journal())

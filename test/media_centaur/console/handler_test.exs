defmodule MediaCentaur.Console.HandlerTest do
  use ExUnit.Case, async: false

  alias MediaCentaur.Console.Handler
  ...
  describe "log/2 reentrancy guard" do
    test "entry with mc_log_source: :buffer is ignored" do
      ...
      Phoenix.PubSub.subscribe(MediaCentaur.PubSub, MediaCentaur.Topics.console_logs())
  ...
  describe "integration: full log path" do
    setup do
      # Buffer is started by the application supervision tree — it is already running.
      # The application also installs :media_centaur_console, so add a separate test
      # handler to avoid interfering with the application handler.
      handler_id = :console_test_handler

      :logger.add_handler(handler_id, MediaCentaur.Console.Handler, %{})

defmodule MediaCentaur.Search.QualityWindowTest do
  use ExUnit.Case, async: true

  alias MediaCentaur.Search.QualityWindow
  ...
  describe "min_at/2 — within patience window with 4K max" do
    test "1 hour after enqueue, effective_min is 4K (insist on 4K)" do
      ...
      assert QualityWindow.min_at(snapshot(%{}), now) == "uhd_4k"
    end
  ...
  describe "min_at/2 — outside patience window" do
    test "exactly at the patience boundary (48h), relaxes to floor" do
      ...
      assert QualityWindow.min_at(snapshot(%{}), now) == "hd_1080p"
    end
  ...
  describe "min_at/2 — patience irrelevant when max_quality is not 4K" do
    test "1080p-only pursuit returns 1080p floor immediately, even within window" do
      now = ~U[2026-01-01 01:00:00Z]
      assert QualityWindow.min_at(snapshot(%{max_quality: "hd_1080p"}), now) == "hd_1080p"
    end
  end
  ...
  describe "min_at/2 — patience disabled (0 hours)" do
    test "returns floor immediately, even with 4K max" do
      now = ~U[2026-01-01 01:00:00Z]
      assert QualityWindow.min_at(snapshot(%{quality_4k_patience_hours: 0}), now) == "hd_1080p"
    end
  end
  ...
  describe "min_at/2 — bounds normalisation" do
    test "treats min == max == 4K as 4K-only, no fallback ever" do
      ...
      s = snapshot(%{min_quality: "uhd_4k", max_quality: "uhd_4k", quality_4k_patience_hours: 0})

      assert QualityWindow.min_at(s, now) == "uhd_4k"
    end
  end
end

defmodule MediaCentaur.Library.ProgressTest do
  @moduledoc """
  Public-API tests for `MediaCentaur.Library.Progress` — the Pillar-2
  ...
  `MediaCentaur.Library.Progress.Events` on the `library:progress`
  ...
  describe "record/3 → get/1 round trip" do
    ...
    test "concurrent writes do not corrupt the record (any single recorded position survives)" do
      ...
      # `MediaCentaur.Playback.MpvSession` /
  ...
  # in "debounced flush". Attaches a handler to `[:media_centaur,

defmodule MediaCentaur.Library.ChangeLogTest do
  ...
  describe "prune/0" do
    test "keeps only the 100 most recent entries" do
      ...
      for _i <- 1..110 do
        Library.ChangeLog.create_change_entry!(%{
          entity_id: movie.id,
      ...
      assert length(Library.ChangeLog.list_recent_changes(150, nil)) == 110

      ChangeLog.prune()

      remaining = Library.ChangeLog.list_recent_changes(150, nil)
  ...
  describe "list_recent_changes" do
    test "returns entries ordered newest-first" do
      ...
      [newest, oldest] = Library.ChangeLog.list_recent_changes(50, nil)

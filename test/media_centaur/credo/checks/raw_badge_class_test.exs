defmodule MediaCentaur.Credo.Checks.RawBadgeClassTest do
  use Credo.Test.Case, async: true

  alias MediaCentaur.Credo.Checks.RawBadgeClass
  ...
  describe "clean code (negative cases)" do
    ...
    test "files outside lib/media_centaur_web/ are not scanned" do
      ~S'''
      defmodule MediaCentaur.NotATemplate do
        @moduledoc "badge badge-info"
        def html_string, do: ~s(<span class="badge badge-info">x</span>)
      end
      '''
      |> to_source_file("lib/media_centaur/not_a_template.ex")
      |> run_check(RawBadgeClass)
      |> refute_issues()
    end
  end
  ...
  describe "violations (positive cases)" do
    ...
    test "raw badge in a class={[\"...\"]} list expression is reported" do
      ~S'''
      defmodule MediaCentaurWeb.ActivityComponent do
        use Phoenix.Component
      ...
      '''
      |> to_source_file("lib/media_centaur_web/live/acquisition_live/activity.ex")
      |> run_check(RawBadgeClass)
      |> assert_issue()
    end

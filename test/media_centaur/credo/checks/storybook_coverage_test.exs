defmodule MediaCentaur.Credo.Checks.StorybookCoverageTest do
  use Credo.Test.Case, async: true

  alias MediaCentaur.Credo.Checks.StorybookCoverage
  ...
  describe "scope" do
    test "ignores files outside lib/media_centaur_web/components/" do
      """
      defmodule MediaCentaurWeb.SomeOtherThing do
      ...
      """
      |> to_source_file("lib/media_centaur_web/some_other_thing.ex")
    ...
    test "ignores files under components/ that have no attr declarations" do
      ...
      """
      |> to_source_file("lib/media_centaur_web/components/detail/logic.ex")
      |> run_check(StorybookCoverage)
      |> refute_issues()
    end
  end
  ...
  describe "v1 coverage — cross-cutting story directories" do
    test "covers a component when its story lives in a cross-cutting area (storybook/composites/)" do
      ...
      """
      |> to_source_file("lib/media_centaur_web/components/modal_shell.ex")
      |> run_check(StorybookCoverage,
        story_paths: [
    ...
    test "errors when no path in the wildcard list matches the component's basename" do
      """
      defmodule MediaCentaurWeb.Components.Orphan do
        attr :label, :string, required: true

        def orphan(assigns), do: ~H"<div></div>"
      end
      """
      |> to_source_file("lib/media_centaur_web/components/orphan.ex")
      |> run_check(StorybookCoverage,
        story_paths: [
          "storybook/composites/modal_shell.story.exs",
          "storybook/core_components/button.story.exs",
  ...
  describe "v2 story shape — namespace" do
    test "errors when story module is not under MediaCentaurWeb.Storybook.*" do
      """
      ...
      |> run_check(StorybookCoverage)
      |> assert_issue(fn issue ->
        assert issue.message =~ ~r/MediaCentaurWeb\.Storybook\./
      end)
    end

    test "passes when story module is under MediaCentaurWeb.Storybook.*" do
      """

defmodule MediaCentaur.SelfUpdate.StagerTest do
  use ExUnit.Case, async: true

  alias MediaCentaur.SelfUpdate.Stager

  @required [
    "bin/media-centaur-install",
    "bin/media_centaur",
  ...
  describe "extract/3 — happy path" do
      ...
      tarball =
        build_tarball(
          Enum.map(@required, fn path -> {path, "stub"} end) ++
            [{"share/defaults/media-centaur.service", "stub"}]
        )

      assert {:ok, ^target} = Stager.extract(tarball, target)
      assert File.exists?(Path.join(target, "bin/media-centaur-install"))
      assert File.exists?(Path.join(target, "bin/media_centaur"))
      assert File.exists?(Path.join(target, "share/systemd/media-centaur.service"))
      assert File.exists?(Path.join(target, "share/defaults/media-centaur.toml"))
    end
    ...
    test "extracts successfully when the tarball lives inside target_dir" do
      ...
      tarball_in_target = Path.join(target, "media-centaur-9.9.9-linux-x86_64.tar.gz")
  ...
  describe "extract/3 — failure paths" do
    test "returns {:error, {:missing_required, [...]}} when required files are absent" do
      ...
      tarball = build_tarball([{"bin/media_centaur", "stub"}])

      assert {:error, {:missing_required, missing}} = Stager.extract(tarball, target)
      assert "bin/media-centaur-install" in missing
      assert "share/systemd/media-centaur.service" in missing
    end

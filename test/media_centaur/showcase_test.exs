defmodule MediaCentaur.ShowcaseTest do
  ...
  describe "History seed shape" do
    ...
    test "writes ≥80 watch_history_events spanning movies and episodes" do
      ...
      events = Repo.all(MediaCentaur.WatchHistory.Event)
    ...
    test "spreads completion events across many days for the heatmap colour ramp" do
      ...
      distinct_days =
        Repo.all(MediaCentaur.WatchHistory.Event)
      ...
      keys =
        Enum.map(releases, fn release ->
          {to_string(release.item.tmdb_id),
           MediaCentaur.ReleaseTracking.tmdb_type_for(release.item.media_type), release.season_number,
           release.episode_number}
        end)

      grab_map = MediaCentaur.Acquisition.statuses_for_releases(keys)

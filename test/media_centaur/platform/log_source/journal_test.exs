defmodule MediaCentaur.Platform.LogSource.JournalTest do
  use ExUnit.Case, async: true

  alias MediaCentaur.Platform.LogSource.Journal
  ...
  describe "available?/1" do
    ...
    test "returns true when unit is set and journalctl is on PATH" do
      ...
      if System.find_executable("journalctl") do
        assert Journal.available?("media-centaur.service")
      end
    end
    ...
    test "returns false when unit is set but journalctl is missing", %{} do
      ...
      assert Journal.available?("media-centaur.service", find_executable: fn _ -> nil end) ==
               false
    end
  end
  ...
  describe "open_port/1 argv shape (port_opener injected)" do
    test "passes --user, -u <unit>, -n 200, -f, --output=short-iso" do
      ...
      assert Journal.open_port("media-centaur.service",
      ...
      assert "--user" in args
      assert "media-centaur.service" in args
      assert "-f" in args
    ...
    test "falls back to /usr/bin/journalctl when find_executable returns nil" do
      ...
      _ =
        Journal.open_port("media-centaur.service",
          port_opener: port_opener,

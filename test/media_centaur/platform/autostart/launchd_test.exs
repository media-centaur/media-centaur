defmodule MediaCentaur.Platform.Autostart.LaunchdTest do
  use ExUnit.Case, async: true

  alias MediaCentaur.Platform.Autostart.Launchd
  ...
  describe "state/1" do
    ...
    test "reports active: false when launchctl print fails (label not loaded)" do
      ...
      cmd =
        fake_cmd(%{
          {"launchctl", ["print", "gui/#{uid}/#{@default_label}"]} =>
            {"Could not find service \"com.media-centaur.app\"\n", 113},
          {"id", ["-u"]} => {"#{uid}\n", 0}
        })
  ...
  describe "status_output/1" do
    test "returns the textual output of `launchctl print` (even on non-zero exit)" do
      cmd = fn binary, _args ->
        case binary do
          "id" -> {"501\n", 0}
          "launchctl" -> {"com.media-centaur.app = {\n\tstate = running\n}\n", 0}
        end
      end

      assert {:ok, output} = Launchd.status_output(cmd_fn: cmd, env_fn: fake_env(%{}))
      assert output =~ "com.media-centaur.app"

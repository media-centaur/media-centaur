defmodule MediaCentaurWeb.Components.UpcomingCardsTest do
  ...
  describe "pending_grab_count/2" do
    ...
    test "excludes releases that already have a target row" do
      ...
      grab_map = %{
        {to_string(item.tmdb_id), MediaCentaur.ReleaseTracking.tmdb_type_for(item.media_type), 1, 1} =>
          {%MediaCentaur.Acquisition.Pursuits.Pursuit{}, grab("seeking")}
      }

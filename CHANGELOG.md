...
- **The systemd journal is on Status → System.** The same log the service writes to disk, in the place the rest of the system health lives. It only starts reading while you have the section open.

### Improved

- **Log history is kept per subsystem.** Each part of the app keeps its own recent lines, so a busy import no longer pushes everything else out. Quiet subsystems now hold hours to days of history where they previously held minutes.
...
- **If you had changed the console buffer size, it resets.** The setting now counts lines per subsystem rather than in total, and it is set on the `/console` page.

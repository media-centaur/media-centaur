...
1. **Every outbound request is built through
   ...
   Prowlarr, qBittorrent, SABnzbd, GitHub, Steam). Prowlarr covers its
   download proxy too: the release-file fetch behind info-hash
   resolution lands on the Prowlarr host, not the indexer. Steam is

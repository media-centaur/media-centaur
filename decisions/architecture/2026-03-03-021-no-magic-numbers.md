...
3. **Promote to user config when the value is a policy decision.** If the user should reasonably want to tune it, add it to `Config.get/1` with a default in `defaults/media-centaur.toml` and a comment explaining what it controls. Examples: `auto_approve_threshold`, `file_absence_ttl_days`.
...
5. **Keep `defaults/media-centaur.toml` complete.** Every key recognized by `Config` must appear in the defaults file with a sensible value and a comment. The defaults file is the documentation for user-facing configuration.
...
* Good, because user-tunable policy decisions are discoverable in `defaults/media-centaur.toml`

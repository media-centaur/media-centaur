...
These rules now live in the `<.button>` component (`MediaCentaurWeb.CoreComponents.button/1`) as named `variant` values: `primary`, `secondary`, `action`, `info`, `risky`, `danger`, `dismiss`, `destructive_inline`, `neutral`, `outline`. The `MediaCentaur.Credo.Checks.RawButtonClass` Credo check (precommit) flags any raw `class="btn ..."` string in templates under `lib/media_centaur_web/`. The button component file (`core_components.ex`) is exempt — it owns the literal `btn` token.

**Standard labels.** `"More info"` is the canonical label for the secondary action that *opens* an entity's detail view from a card or hero — the hero CTA pair is **Play** + **More info**. *Inside* the detail modal, the secondary toggle reveals the manage sub-view (files, external ids, rematch) and is labelled **Manage** (with `hero-cog-6-tooth-mini`). It keeps that label while the sub-view is open and shows the open state as `aria-pressed`; the way out is the view control named for its destination ("Overview", "Episodes"), never a "Back" relabel — one control, one meaning (amended 2026-09-05; UIDR-019 made the modal's regions the navigation model). Avoid synonyms (`Details`, `More`, `Info`, `Settings`, `Edit`) that drift the visual language.

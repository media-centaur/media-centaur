...
> the tracking controls render only for a listed title; a title not on the
> list offers only the bookmark of point 5, which every title surface now
> wears beside its primary verb (`Title.WatchlistToggle`). Point 3 is
> narrowed accordingly; point 5 stands and is generalised.

...
* **Watchlist** — every title intent at List or above. Not a tab: the
  ...
  are others.
* **Bookmark** — the one-click beside a title's primary verb that lists it
  (accessible name *Add to watchlist*); the only verb a title not on the
  list (Off or Ignored) offers. Sets List, nothing else; a marker at
  Follow and above.
...
icon over a button, 2026-09-11); `IntentControl.control_form/1` answers
`:none` / `:ignored` / `:controls`, so the tracking block is empty until a
title is listed. Recommend is the paper-plane icon on both surfaces. UIDR-039 written, UIDR-036
...
* A title not on the watchlist offers the bookmark (and, on the Feed

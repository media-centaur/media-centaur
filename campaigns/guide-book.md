...
their own problems. Chapters are authored as portable markdown and rendered in-app;
the guide is reachable only from Settings → System for now. Worth doing because the
software rewards understanding and there is currently no in-app place that explains it.
...
1. **Write the Phase-0 TDD plan** — `docs/superpowers/plans/2026-06-15-guide-book-phase-0.md` (content format + renderer + route + layout + Settings link + pilot chapter). Resolve the markdown→HEEx rendering approach there (existing dep vs. compile-time vs. runtime).
2. **Phase 0 — infrastructure + pilot chapter.**

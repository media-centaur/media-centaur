...
`settings_field/1` and `settings_input/1` are the label/control unit and the house input inside a connection row's form; `settings_disclosure/1` hides rare content. Rows are wrapping flex lines: the text block asks for a 16rem measure and a control that no longer fits beside it drops beneath, left-aligned; a card action or a connection row's state and actions drop right-aligned. Each row story carries a `:narrow` variation at the width a half-width window leaves at 2× UI scale. Readiness has one owner, `MediaCentaur.IntegrationHealth`: Test and Save and test call `verify/1` and the row follows its broadcasts; `SettingsLive.ConnectionState` is the pure projection the row draws.

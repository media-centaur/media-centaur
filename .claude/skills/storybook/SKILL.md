...
| **Theme scoping** | `html.psb:not(.psb\:dark)` resets to light; `.psb-variation-block .media-centaur` restores dark | Storybook chrome stays readable in either color mode; component previews show our real theme. Don't touch this without reading [`references/sandboxing.md`](references/sandboxing.md). |
| **Color mode** | `color_mode: true` in the backend | Header picker (light/dark/system) for the chrome. It also adds a `dark` class to the sandbox, which is inert — the app is dark-only, so previews render dark in every mode. |
...
| Storybook chrome shows light-on-light text | Daisy `:root { color-scheme: dark }` leaks into storybook chrome | The `html.psb:not(.psb\:dark)` override in `assets/css/app.css` resets this. Don't remove it, and don't drop the `:not()` — it's what hands dark mode back to storybook's own chrome styling. |
| A chrome-level `background`/`color` override in `app.css` does nothing | Storybook imports our css as `@import url(app.css) layer(app)`; its own `psb:*` utilities sit in a later layer and win regardless of specificity | Only properties storybook's chrome doesn't set itself (e.g. `color-scheme`) land. Style the chrome through the color-mode picker, not by fighting the layer. |

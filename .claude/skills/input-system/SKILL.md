...
- **The wheel takes scroll authority.** A wheel event is the pointer claiming the scroll: the orchestrator cancels every glide where it stands (`writer.cancelScrollMotion()`) and switches to mouse mode. While the method is mouse, restores the user didn't ask for (post-patch reconciles, cursor-start seeding) pass `{ reveal: false }` to the writer's focus calls — focus survives the patch, the viewport never moves. The next keypress flips the method back before its action runs, so cursor navigation reveals as before. Never add a reveal that can fire in mouse mode without routing through `_restoreOpts()`.
